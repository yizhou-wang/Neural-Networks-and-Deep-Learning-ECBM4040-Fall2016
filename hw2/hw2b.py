...
class myMLP(object):
    ...
    def __init__(self, rng, input, n_in, n_hidden, n_out, n_hl, act_fun):
        """Initialize the parameters for the multilayer perceptron
        ...
        L2_sqr_sum = 0
        params_sum = []                               
        self.hiddenLayer = range(n_hl)
        
        for i in range(n_hl):
            
            print ('... building', i+1, 'hidden layer')
            ...
            L1_sum = L1_sum + abs(self.hiddenLayer[i].W).sum()
            L2_sqr_sum = L2_sqr_sum + (self.hiddenLayer[i].W ** 2).sum()
            params_sum = params_sum + self.hiddenLayer[i].params
                

        # The logistic regression layer gets as input the hidden units
        # of the hidden layer
        self.logRegressionLayer = LogisticRegression(
            input=self.hiddenLayer[n_hl-1].output,
            n_in=n_hidden,
            n_out=n_out
        )
        # L1 norm ; one regularization option is to enforce L1 norm to
        ...
        # the parameters of the model are the parameters of the two layer it is
        # made out of
        self.params = params_sum + self.logRegressionLayer.params
...
def test_mlp(learning_rate=0.01, L1_reg=0.00, L2_reg=0.0001, n_epochs=1000,
             batch_size=20, n_hidden=500, verbose=False, n_hl=1, act_fun='tanh' ):
    """
